...
## Modifed by jaewook lee to test iCPS Network
...
from ICPS_Gateway.nsh.decode import *  # noqa
from ICPS_Gateway.nsh.encode import *  # noqa
...
class MyVxlanNshEthClient(MyNshBaseClass):
    ...
    try:
		...
		# Building client IP packet to send to SFF
            packet = build_nsh_eth_header(self.encapsulate_header_values,self.base_header_values,self.ctx_header_values,self.ethernet_values)
		...
		#logger.info("Sending Video packet to SFF UPF-edge(%s) at %s", (self.remote_sff_ip, self.remote_sff_port), self.sendingtime)
		...
		#logger.info("Receiving Video packet from UPF: %s at %s", addr, self.receivingtime)
            RTT = self.receivingtime - self.sendingtime
		#logger.info("Round Trip Time for iCPS Service from GW to iCPS Server: %s Second", RTT)
            print(RTT)
            logger.debug("Packet dump: %s", binascii.hexlify(data))
		# Decode all the headers
            decode_vxlan(data, self.server_vxlan_values)
