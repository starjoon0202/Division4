    ...
    def process_datagram(self, data, addr):
        ...
        if nsh_decode.is_data_message(data):
            ...
            if nsh_decode.is_vxlan_nsh_legacy_message(data):
                ...
                addr = '192.168.100.106'
                addr_l = list(addr)
                addr_l[1] = 6633
                addr = tuple(addr_l)
 #               logger.info('%s service forwards packet to %s:', self.service_type, addr)
                time.sleep (0.00)
                if self.server_base_values.service_path == 1
                    logger.info('MCI service packet forwards packet to Edge_Cloud_Switch %s:', addr)
                    self.transport.sendto(rw_data, ('192.168.100.116',6633))
                 elif self.server_base_values.service_path == 2 :
                    logger.info('eMBB service packet forwards packet to Remote UPF %s:',  addr)
                    self.transport.sendto(rw_data, ('192.168.100.126',6633))
                  elif self.server_base_values.service_path == 3 :
                     logger.info('eMBB service packet forwards packet to Edge_Cloud_Switch %s:',  addr)
                     self.transport.sendto(rw_data, ('192.168.100.116',6633))
...
class MyUPFService(BasicService):
    def __init__(self, loop):
        super(MyUPFService, self).__init__(loop)

        self.service_type = UPF
